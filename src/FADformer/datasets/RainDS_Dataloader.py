...
class TrainData_for_rainstreak(data.Dataset):
    def __init__(self, crop_size, train_data_dir, only_h_flip=False):
        super().__init__()
        train_list_rain = './datasets/RainDS_real/train_set/LIST.TXT'
    ...
    def get_images(self, index):
        ...
        rain_path = os.path.join(self.train_data_dir, 'rainstreak', rain_name)
...
class TrainData_for_rainstreak_raindrop(data.Dataset):
    def __init__(self, crop_size, train_data_dir, only_h_flip=False):
        super().__init__()
        train_list_rain = './datasets/RainDS_real/train_set/LIST.TXT'
    ...
    def get_images(self, index):
        rain_name = self.rain_names[index]
        gt_name = self.gt_names[index]
        rain_path = os.path.join(self.train_data_dir, 'rainstreak_raindrop', rain_name)
        gt_path = os.path.join(self.train_data_dir, 'gt', gt_name)
        rain_img = imfrombytes(read_img255(rain_path), float32=True)
        gt_img = imfrombytes(read_img255(gt_path), float32=True)
        [rain_img, gt_img] = augment([rain_img, gt_img], size=self.crop_size, edge_decay=0.,
...
class TrainData_for_raindrop(data.Dataset):
    def __init__(self, crop_size, train_data_dir, only_h_flip=False):
        super().__init__()
        train_list_rain = './datasets/RainDS_real/train_set/LIST.TXT'
    ...
    def get_images(self, index):
        rain_name = self.rain_names[index]
        gt_name = self.gt_names[index]
        rain_path = os.path.join(self.train_data_dir, 'raindrop', rain_name)
        gt_path = os.path.join(self.train_data_dir, 'gt', gt_name)
        rain_img = imfrombytes(read_img255(rain_path), float32=True)
        gt_img = imfrombytes(read_img255(gt_path), float32=True)
        [rain_img, gt_img] = augment([rain_img, gt_img], size=self.crop_size, edge_decay=0.,
...
class Test_for_rainstreak(data.Dataset):
    def __init__(self, val_data_dir, flag=True):
        super().__init__()

        val_list_haze = './datasets/RainDS_real/test_set/LIST.TXT'
    ...
    def get_images(self, index):
        rain_name = self.rain_names[index]
        gt_name = self.gt_names[index]
        rain_path = os.path.join(self.val_data_dir, "rainstreak", rain_name)
...
class Test_for_rainstreak_raindrop(data.Dataset):
    def __init__(self, val_data_dir, flag=True):
        super().__init__()
    ...
    def get_images(self, index):
        rain_name = self.rain_names[index]
        gt_name = self.gt_names[index]
        rain_path = os.path.join(self.val_data_dir, "rainstreak_raindrop", rain_name)
        gt_path = os.path.join(self.val_data_dir, "gt", gt_name)
        rain_img = imfrombytes(read_img255(rain_path), float32=True)
        gt_img = imfrombytes(read_img255(gt_path), float32=True)
        h, w, c = rain_img.shape
...
class Test_for_raindrop(data.Dataset):
    def __init__(self, val_data_dir, flag=True):
        super().__init__()
    ...
    def get_images(self, index):
        rain_name = self.rain_names[index]
        gt_name = self.gt_names[index]
        rain_path = os.path.join(self.val_data_dir, "raindrop", rain_name)
        gt_path = os.path.join(self.val_data_dir, "gt", gt_name)
        rain_img = imfrombytes(read_img255(rain_path), float32=True)
        gt_img = imfrombytes(read_img255(gt_path), float32=True)
        h, w, c = rain_img.shape
